...
def router_message(client, text: str, user: str):
    if text == "测试":  # 发送测试即可返回相同内容
        send_text(text, user, client, "open_id")  # 传入text变量，user变量，将接收到的消息复读一遍发送
    elif text == "查询知识库列表":  # 添加了新的逻辑分支
        for name in list_space_request(client):  # 遍历list_space_request(client)的列表
            send_text(text=name, user=user, client=client, receive_id_type="open_id")  # 调用text，user，client
...
def router_event(client, user, event_name=None):
    # todo 可通过卡片进行逐步选择
    if event_name == '测试':
        send_text(event_name, user, client, "open_id")
        ...
        trigger_birthday_judge(client=client, user=user)
...
def schedule_event():
    ...
    trigger_birthday_judge(client, "")


def search_space_id(client, space_name, user):
    items = list_space_request(client)
    for item in items:
        if item.name == space_name:
            return item.space_id
    send_text(text=f"找不到知识库 '{space_name}' ", user=user, client=client, receive_id_type="open_id")
    return None


def search_node_token(client, space_id, node_name, user):
    nodes = list_space_node(space_id, client)
    for node in nodes:
        if node_name == node.title:
            return node
    send_text(text=f"找不到知识库节点 '{node_name}' ", user=user, client=client, receive_id_type="open_id")


def search_table(client, app_token, table_name, user):
    tables = list_app_table(app_token, client)
    for table in tables:
        if table_name == table.name:
            return table
    send_text(text=f"找不到数据表 '{table_name}' ", user=user, client=client, receive_id_type="open_id")


def search_table_view(client, app_token, table_id, table_view_name, user):
    views = list_app_table_view(app_token, table_id, client)
    for view in views:
        if table_view_name == view.view_name:
            return view
    send_text(text=f"找不到数据表视图 '{table_view_name}' ", user=user, client=client, receive_id_type="open_id")


def trigger_birthday_judge(client, user):
    # 遍历生日，发送提醒
    #  新入口，实现定时调用能力

    # 1. 定位到知识库 - 对应节点 - 对应多维表格 - 对应视图
    space_id = search_space_id(client=client, space_name="成长小组知识库", user=user)
    app_token = search_node_token(client=client, space_id=space_id, node_name="【新青年】管理表格", user=user).obj_token
    table_id = search_table(client=client, app_token=app_token, table_name="新青年活动事件总表", user=user).table_id
    ...
    send_text(output_text, user, client, "open_id")
...
def search_node_in_space(client, space_name, user):
    ...
    if space_id:
        ...
        for node in nodes:  # 遍历node，传入title和url
            ...
            text += title + url + "\\n"  # todo 为什么传"\\n"
        send_text(text, user, client, "open_id")  # 调用send text
    else:
        # 变量没有匹配时的兜底逻辑
        send_text(text=f"找不到知识库 '{space_name}' ", user=user, client=client, receive_id_type="open_id")
